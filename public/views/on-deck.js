...
export function createOnDeckView({ api, ui, controlsRoot } = {}) {
  ...
  function configuredPageSize() { return clamp(Number(settings.views?.onDeck?.itemsPerPage), 1, 250, 12); }
  ...
  function updateControlsState() { if (!controlsRoot || !isVisible) return; const meta = controlsRoot.querySelector('[data-deck-filter-meta]'); if (meta) meta.textContent = `${activeTypes.size}/3 · ${viewMode === 'grid' ? currentSize().slice(0,1).toUpperCase() : 'Lista'}${activeGroupIds.size ? ` · ${activeGroupIds.size} grupo(s)` : ''}`; }
  ...
  function downloadCsv() {
    ...
    link.href = url;
    link.download = `kiosko-deck-${new Date().toISOString().slice(0,10)}.csv`;
