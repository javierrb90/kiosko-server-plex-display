...
export function createNotificationsView() {
  ...
  async function load(page) { const response = await fetch(`/api/notifications?page=${page}&limit=5`); const data = await response.json(); state = data; render(el, data); }
  return {
    id: "notifications",
    mount(target) {
      el = target;
      el.innerHTML = `<div class="notifications-shell"><header><p class="eyebrow">CENTRO DE NOTIFICACIONES</p><div><span class="notification-count">0 EVENTOS</span><span class="notification-page"></span></div></header><div class="notification-list"></div><footer><button class="nav-btn" data-direction="-1" aria-label="Página anterior">‹</button><button class="nav-btn" data-direction="1" aria-label="Página siguiente">›</button></footer></div>`;
      el.querySelectorAll(".nav-btn").forEach(button => button.addEventListener("click", () => { const next = state.page + Number(button.dataset.direction); if (next >= 1 && next <= state.totalPages) load(next); }));
    },
