  ...
  async function runAction(root, action) {
    ...
    try {
      ...
      if (action === 'toggle-backlog') {
        if (isInBacklog(item, currentContext)) { await api(`/api/items/${canonical}/backlog`, { method: 'DELETE' }); item.states = { ...(item.states || {}), inBacklog: false }; toast('Quitado del Backlog'); if (currentContext === 'backlog') currentContext = 'removed'; }
      ...
      if (action === 'toggle-deck') {
        if (isInDeck(item, currentContext)) { await api(`/api/items/${canonical}/deck`, { method: 'DELETE' }); item.states = { ...(item.states || {}), inOnDeck: false }; toast('Quitado de On Deck'); if (currentContext === 'on-deck') currentContext = 'removed'; renderBody(root); return; }
