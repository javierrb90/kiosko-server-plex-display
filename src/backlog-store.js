...
export class CompletionStore {
  ...
  async removeByCanonicalId(canonicalId) { const index = this.items.findIndex(entry => entry.canonicalId === canonicalId); if (index < 0) return null; const [removed] = this.items.splice(index, 1); await this.persist(); return removed; }
