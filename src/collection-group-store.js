...
function normalizeGroup(input = {}, existing = {}) {
  ...
  const mode = ["manual", "dynamic", "mixed"].includes(input.mode || existing.mode)
    ? (input.mode || existing.mode)
    : "manual";
  const match = ["all", "any"].includes(input.match || existing.match)
    ? (input.match || existing.match)
    : "all";
  const rules = Array.isArray(input.rules)
    ? input.rules.map(normalizeRule).filter(Boolean)
    : (existing.rules || []);
  return {
...
export class CollectionGroupStore {
  ...
  async init() {
    ...
    try {
      ...
      this.groups = Array.isArray(parsed) ? parsed.map(group => normalizeGroup(group, group)) : [];
    } catch (error) {
